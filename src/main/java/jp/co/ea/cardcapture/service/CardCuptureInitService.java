...
public class CardCuptureInitService {
	...
	public PlayerSession initGame() {
		...
		if ("DEBUG".equals(appMode)) {
			// テストデッキ作成
			enemyDeck.createDeck(creatMockEnemyDeck());
			playerDeck.createDeck(creatMockPlayerDeck());
		} else {
			// 敵デッキ作成
		...
		// PlayerHandsの作成
		GameDeck playerHands = pSession.getPlayerHands();
		for (int i = 0; i < 4; i++) {
...
//	}
	 ...
	 * @return
	 */
	private List<Integer> creatMockPlayerDeck() {
	 ...
	 * @return
	 */
	private List<Integer> creatMockEnemyDeck() {
